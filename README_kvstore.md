...
# KVStore — تمرین سیستم‌های توزیع‌شده (نسخهٔ فایل‌محور + قابلیت‌های اختیاری)

این پروژه یک سرویس سادهٔ **Key–Value** است که با **Go** نوشته شده و **بدون دیتابیس خارجی** کار می‌کند. داده‌ها در فایل‌های `JSONL` ذخیره می‌شوند و بعد از ری‌استارت باقی می‌مانند.  
سرویس Dockerized است (ایمیج کم‌حجم با multi-stage + distroless) و سه **گزینهٔ اختیاری** هم پیاده‌سازی شده است:
